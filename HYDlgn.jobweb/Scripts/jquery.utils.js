(function (uicontrolLib, $, $imask) {
    ...
    function resetseq(row, seq) {
        var offset = 0;
        if (!seq) {
            seq = $(row).prev().find('td span').text();
            if (!seq) {
                seq = $(row).find('td span').text();
                offset = -1;

            }
            var seqint = parseInt(seq) + offset;
            if (!isNaN(seqint)) {
                seqint = seqint + 1;
                $(row).find('td span').text(seqint);
                $(row).find('td span').next().val(seqint);
                $(row).nextAll().each(function (ni, el) {
                    seqint = seqint + 1;
                    $(el).find('td span').text(seqint);
                    $(el).find('td span').next().val(seqint);
                });
            }
        }
    }
    ...
    function getBaseSetting() {
        var setting = {
            ...
            SearchSep: $('#base_SearchSep').val(),
            BaseUrl: $('#base_BaseUrl').val(),
        };
        sessionStorage.setItem('basesetting', JSON.stringify(setting));
        var basesettingjson = sessionStorage.getItem('basesetting');
        if (basesettingjson) {
            var basesetting = JSON.parse(basesettingjson);
            return basesetting;
        }
    ...
    uicontrolLib.Core = {
        ...
        sanitizeUrlVanilla: sanitizeUrlVanilla,
        dndlogdrop: dndlogdrop,
        getBaseSetting: getBaseSetting,
    };
